...
target <- c("Employment", "Country", "EdLevel", "YearsCode", "DevType", "OrgSize", "LanguageHaveWorkedWith", "Age", "Gender", "ConvertedCompYearly")
...
df_clean <- df %>%
      mutate(
        ...
        Gender = as.factor(ifelse(grepl("Man", Gender, ignore.case=TRUE), "Man", ifelse(grepl("Women", Gender, ignore.case=TRUE), "Women", ifelse(grepl("Non-binary", Gender, ignore.case=TRUE), "Non-binary", NA)))),
...
df_clean[df_clean == ""|df_clean == "Prefer not to say"|df_clean == "I prefer not to say"] <- NA
